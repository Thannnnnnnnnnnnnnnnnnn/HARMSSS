...
if (isset($_GET['id']) && isset($_GET['token'])) {
    ...
    if ($resultFound) {
        ...
        $transac->execute();
            ...
            $message = "Reservation #{$reservation_id} has been Confirmed.";
    ...
    // audit d2
        ...
        if ($stored_token === $token) {
            if ($reservationStatus === 'Cancelled') {
                ...
                $error = "Your reservation has already been cancelled. To secure your stay, please make a new booking.";
            } else {
...
<body class='min-h-screen bg-gradient-to-br from-[#f7f4ef] to-[#fff] flex items-center justify-center px-4'>
    ...
    <div class='bg-white border border-[#e0d6c6] shadow-xl rounded-3xl p-10 max-w-xl w-full text-center'>
        ...
        <h1 class='text-3xl text-green-700 font-bold mb-4'>Reservation Confirmed</h1>
        <p class='text-[#5c4b37] text-lg mb-2'>Thank you, <span class='font-semibold'><?= htmlspecialchars($guestName) ?></span>!</p>
        <p class='text-[#5c4b37] text-lg mb-2'>Your reservation <span class='font-semibold'>#<?= $reservationId ?></span> has been confirmed.</p>
...
<?php else: ?>
    <div class='bg-white border border-red-400 text-red-700 shadow-xl rounded-3xl p-10 max-w-xl w-full text-center'>
        <h1 class='text-3xl font-bold mb-4'>Reservation Not Confirmed</h1>
