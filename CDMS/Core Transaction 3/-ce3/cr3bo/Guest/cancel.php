...
if (isset($_GET['id']) && isset($_GET['token'])) {
        ...
        $transac->execute();
         // audit d2
            ...
            renderPage(
                ...
                "Reservation Cancelled",
                "Your reservation <span class='font-semibold'>#{$reservation_id}</span> has been successfully cancelled.",
                "red"
            );
        } else {
            renderPage(
                ...
                "<svg class='mx-auto h-16 w-16 text-red-600' fill='none' stroke='currentColor' stroke-width='1.5' viewBox='0 0 24 24'><path stroke-linecap='round' stroke-linejoin='round' d='M12 9v2m0 4h.01m-6.938 4h13.856c1.054 0 1.635-1.14 1.077-2.05L13.077 4.95c-.527-.91-1.86-.91-2.387 0L3.005 17.95C2.447 18.86 3.028 20 4.082 20z' /></svg>",
                "Error",
                "There was a problem cancelling the reservation. Please try again.",
                "red"
            );
        }
        $stmt->close();
    ...
    renderPage(
        ...
        "<svg class='mx-auto h-16 w-16 text-red-600' fill='none' stroke='currentColor' stroke-width='1.5' viewBox='0 0 24 24'><path stroke-linecap='round' stroke-linejoin='round' d='M12 9v2m0 4h.01m-6.938 4h13.856c1.054 0 1.635-1.14 1.077-2.05L13.077 4.95c-.527-.91-1.86-.91-2.387 0L3.005 17.95C2.447 18.86 3.028 20 4.082 20z' /></svg>",
        "Missing Parameters",
        "Reservation ID or cancellation token is missing.",
        "red"
    );
}
?>
