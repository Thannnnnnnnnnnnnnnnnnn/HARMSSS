...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['create_channel'])) {
    ...
    if ($stmt->execute()) {
        ...
        logTransaction($usmDb, $department_id, $user_id, 'Insert', " Insert Booking Channel Data");
        logAudit($usmDb, $department_id, $user_id, 'Insert', 'Core 3', 'Booking', " Insert Booking Channel Data");
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['update_channel'])) {
    ...
    if (!empty($binago)) {
        ...
        if ($stmt->execute()) {
            ...
            logTransaction($usmDb, $department_id, $user_id, 'Update', "Update Booking Channel Data");
            logAudit($usmDb, $department_id, $user_id, 'Update', 'Core 3', 'Booking', " Update Booking Channel Data");
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['delete_channel'])) {
    ...
    if ($stmt->execute()) {
        ...
        logTransaction($usmDb, $department_id, $user_id, 'Delete', " Delete Booking Channel Data");
        logAudit($usmDb, $department_id, $user_id, 'Delete', 'Core 3', 'Booking', " Delete Booking Channel Data");
    ...
    <div class="bg-gradient-to-b from-white to-orange-100 min-h-screen p-8">
        ...
        <div class="flex items-center justify-between mt-4">
            <div class="text-sm text-gray-700">
                Showing <?= min($itemsPerPage, $totalPages) ?> to <?= $itemsPerPage ?> Channels
            </div>
...
function openViewModal(button) {
    ...
    document.getElementById('modalChannelName').textContent = details.name;
...
function openUpdateModal(button) {
    ...
    document.getElementById('updateChannelName').value = details.name;
