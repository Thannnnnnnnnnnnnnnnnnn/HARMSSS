...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['update_guest'])) {
    ...
    if (!empty($binago)) {
        ...
        if ($stmt->execute()) {
            ...
            logTransaction($usmDb, $department_id, $user_id, 'Update', "Update Guest Preference Data");
            logAudit($usmDb, $department_id, $user_id, 'Update', 'Core 3', 'Booking', "Update Guest Preference Data");
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['delete_preference'])) {
    ...
    if ($stmt->execute()) {
        ...
        logAudit($usmDb, $department_id, $user_id, 'Delete', 'Core 3', 'Booking', "Delete Guest Preference Data");
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['create_preference'])) {
    $guest_id = trim($_POST['guest_id']);
    $type = trim($_POST['type']);
    $description = trim($_POST['description']);

    $stmt = $connection->prepare("INSERT INTO guest_preferences (GuestID, PreferenceType, PreferenceDetail) VALUES (?, ?, ?)");
    $stmt->bind_param("iss", $guest_id, $type, $description);

    if ($stmt->execute()) {
        $new_id = mysqli_insert_id($connection);
        $_SESSION['success_message'] = "Guest Preference added successfully!";
        
        $notification = $notif->prepare("INSERT INTO notifications (User_ID, notifType, message, status, Department, date_sent) 
            VALUES (?, 'create', ?, 'Unread', 'Core 3', NOW())");
        $message = "- $role created Guest Preference ID #$new_id.";
        $notification->bind_param("ss", $user_id, $message);
        $notification->execute();

        logTransaction($usmDb, $department_id, $user_id, 'Insert', "Insert Guest Preference Data");
        logAudit($usmDb, $department_id, $user_id, 'Insert', 'Core 3', 'Booking', "Insert Guest Preference Data");

        header("Location: guest.php");
        exit();
    } else {
        $_SESSION['error_message'] = "Error adding preference: " . $stmt->error;
    }
    $stmt->close();
}
...
<table class="min-w-full text-[12px] text-center text-[#594423]">
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            $buttonData = [
                'id' => $row['PreferencesID'],
                'type' => $row['PreferenceType'],
                'description' => $row['PreferenceDetail'],
                'gue' => $row['GuestName']
            ];
        ...
        echo "<tr><td colspan='9' class='text-center py-4'>No Preferences Found.</td></tr>";
    }
    ?>
    </tbody>
</table>
...
<div class="flex items-center justify-between mt-4">
    ...
    Showing <?= min($itemsPerPage,$totalPages)?> to <?= $itemsPerPage?> Preferences
    </div>
...
<div id="insertModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden z-50">
    <div class="bg-white rounded-lg shadow-lg p-6 w-full max-w-lg relative">
        <h2 class="text-xl font-bold mb-4">Add New Preference</h2>
        <form method="POST" class="space-y-4">
            <div>
                <label class="block text-sm font-medium">Guest ID</label>
                <?php
                $guest_query = $connection->query("SELECT GuestID, guest_name FROM `$db_guest`.guests");
                echo '<select name="guest_id" required class="w-full border p-2 rounded">';
                echo '<option value="">Select Guest</option>';
                while ($guest = $guest_query->fetch_assoc()) {
                    echo "<option value='{$guest['GuestID']}'>{$guest['guest_name']}</option>";
                }
                echo '</select>';
                ?>
            </div>
            <div>
                <label class="block text-sm font-medium">Preference Type</label>
            ...
            <div>
                <label class="block text-sm font-medium">Preference Detail</label>
            ...
            <div class="flex justify-end space-x-2 pt-2">
                ...
                <button type="submit" name="create_preference" class="px-4 py-2 bg-[#f1ddbe] text-black rounded">
                    Submit
                </button>
            </div>
        </form>
...
<div id="viewModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden z-50" onclick="closeViewModal()">
    <div class="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-md relative animate__animated animate__fadeInDown" onclick="event.stopPropagation()">
        ...
        <button onclick="closeViewModal()" class="text-gray-400 hover:text-gray-600 text-xl">×</button>
        </div>
...
<div id="deleteModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden z-50" onclick="closeDeleteModal()">
    ...
    <p class="mb-4">Are you sure you want to delete this preference?</p>
...
function openUpdateModal(button) {
    ...
    document.getElementById('updateChannelId').value = details.id;
    document.getElementById('updateChannelType').value = details.type;
...
function openViewModal(button) {
    const details = JSON.parse(button.getAttribute('data-details'));
    document.getElementById('modalgus').textContent = details.gue;
    document.getElementById('modalid').textContent = details.id;
    document.getElementById('modalChannelType').textContent = details.type;
...
function openDeleteModal(button) {
    const details = JSON.parse(button.getAttribute('data-details'));
    document.getElementById('deleteChannelId').value = details.id;
