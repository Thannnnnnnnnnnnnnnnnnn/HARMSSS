...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit_guest_reservation'])) {
    ...
    $checkStmt = $bookingDb->prepare("
        ...
        WHERE g.email = ? AND b.ReservationStatus IN ('Pending')
    ");
    if ($checkStmt === false) {
...
<body>
  <div class="email-container">
    ...
    <div class="details">
      <p>We are pleased to confirm your Booking under the <strong>ID #$id</strong>.</p>
      <p>You have selected <strong>Room $room_number</strong>, tailored for your comfort and relaxation.</p>
      <p>This Booking accommodates <strong>$num_guests guest(s)</strong>, ensuring an exquisite experience for all.</p>
...
EOD;
            ...
            try {
                ...
                $mail->Password = 'aoiiurqgjoowlhnf';
                $mail->SMTPSecure = 'tls';
                $mail->Port = 587;
    ...
    <div id="booking-complete" style="<?php echo $showBookingComplete ? 'display: block;' : 'display: none;'; ?>">
      ...
      <div class="text-center">
        ...
        <div class="bg-gold-text text-white p-4 rounded-md shadow-md">
          <p class="text-base">Booking Number: <span class="font-bold">AVL-<?php echo date('Ymd') . '-' . str_pad($id, 3, '0', STR_PAD_LEFT); ?></span></p>
