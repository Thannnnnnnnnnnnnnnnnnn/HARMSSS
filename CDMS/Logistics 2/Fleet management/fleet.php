<?php 
   ...
   $stmt = $conn->prepare("SELECT * FROM fleet");
   $stmt->execute();
...
<body>
    <div class="flex min-h-screen w-full">
        ...
        <div class="main w-full bg-[#FFF6E8] md:ml-[320px] overflow-y-auto"> <!-- Added overflow-y-auto -->
            ...
            <!-- main Content -->
            <?php if (!empty($fleets)) : ?>
            <h2 class="pt-6 pl-6 text-xl font-medium mb-4 text-gray-800">Fleet </h2>
...
<tbody>
    <?php foreach ($fleets as $row) : ?>
        <tr>
            <td><?= htmlspecialchars($row['FleetID']) ?></td>
            <td><?= htmlspecialchars($row['FleetName']) ?></td>
            <td><?= htmlspecialchars($row['ManagerID']) ?></td>
            <td><?= htmlspecialchars($row['Employee_name']) ?></td>
            <td><?= htmlspecialchars($row['Model']) ?></td> 
            <td><?= htmlspecialchars($row['Capacity']) ?></td> 
            <td><?= htmlspecialchars($row['NumberOfItems']) ?></td> 
            <td>
<!-- Add Fleet Button -->
<button type="button" id="addFleetBtn" class="btn btn-success ms-2" data-bs-toggle="modal" data-bs-target="#addFleetModal">
...
data-bs-target="#newViewModal"
data-fleet-id="<?php echo htmlspecialchars($row['FleetID']); ?>"
data-fleet-name="<?php echo htmlspecialchars($row['FleetName']); ?>"
data-manager-id="<?php echo htmlspecialchars($row['ManagerID']); ?>"
...
data-numitems="<?php echo htmlspecialchars($row['NumberOfItems']); ?>">
...
data-fleetid="<?php echo htmlspecialchars($row['FleetID']); ?>"
data-fleetname="<?php echo htmlspecialchars($row['FleetName']); ?>"
...
<?php endif; ?>
<!-- Add Fleet Modal -->
<div id="addFleetModal" class="modal fade" tabindex="-1">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h2 class="modal-title">Add New Fleet</h2>
        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
      </div>
      <div class="modal-body">
        <form action="add.php" method="POST">
          <div class="mb-3">
            <label for="FleetID" class="form-label">Fleet ID:</label>
            <input type="text" class="form-control" name="FleetID" required>
          </div>
          <div class="mb-3">
            <label for="FleetName" class="form-label">Fleet Name:</label>
            <input type="text" class="form-control" name="FleetName" required>
          </div>
          <div class="mb-3">
          ...
          <div class="mb-3">
            ...
            <input type="text" class="form-control" name="NumberOfItems" required>
          </div>
...
<!-- View Modal -->
<div class="modal fade" id="newViewModal" tabindex="-1" aria-labelledby="newViewModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header bg-blue-600 text-white">
                <h5 class="modal-title" id="newViewModalLabel">Fleet Details</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <p><strong>Fleet ID:</strong> <span id="viewFleetID"></span></p>
                <p><strong>Fleet Name:</strong> <span id="viewFleetName"></span></p>
                ...
                <p><strong>Number of Items:</strong> <span id="viewNumberOfItems"></span></p>
            </div>
...
<div class="modal fade" id="editModal" tabindex="-1" aria-labelledby="editModalLabel" aria-hidden="true">
    ...
    <div class="modal-content">
    <div class="modal-header">
        <h5 class="modal-title" id="editModalLabel">Edit Fleet</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
    </div>
    <div class="modal-body text-left px-4 py-3">
<!-- Hidden input to store original Fleet ID -->
<input type="hidden" id="oldFleetID" name="old_fleet_id">

<div class="mb-3">
    <label for="editFleetID" class="form-label fw-bold">Fleet ID</label>
    <input type="text" class="form-control" id="editFleetID" name="fleet_id">
</div>

        <div class="mb-3">
            <label for="editFleetName" class="form-label fw-bold">Fleet Name</label>
            <input type="text" class="form-control" id="editFleetName" name="fleet_name">
        </div>

        <div class="mb-3">
            <label for="editManagerID" class="form-label fw-bold">Manager ID</label>
            <input type="text" class="form-control" id="editManagerID" name="manager_id">
        </div>

        <div class="mb-3">
            <label for="editEmployeeName" class="form-label fw-bold">Employee Name</label>
...
<div class="mb-3">
    <label for="editCapacity" class="form-label fw-bold">Capacity</label>
    <input type="text" class="form-control" id="editCapacity" name="capacity">
</div>
<div class="mb-3">
    <label for="editNumberOfItems" class="form-label fw-bold">Number of Items</label>
    <input type="text" class="form-control" id="editNumberOfItems" name="number_of_items">
</div>
...
<div class="modal fade" id="deleteModal" tabindex="-1" aria-labelledby="deleteModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p>Are you sure you want to delete <strong id="deleteFleetName"></strong>?</p>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                <form method="POST" action="delete.php">
                    <input type="hidden" name="fleet_id" id="deleteFleetID">
