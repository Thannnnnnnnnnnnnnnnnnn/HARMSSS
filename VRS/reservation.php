...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $status = $_POST['status'];
                ...
                // NEW CODE STARTS HERE - Automatic Vehicle and Driver Assignment
            // Fetch an available vehicle based on type
            $vehicleQuery = "SELECT VehicleID FROM vehicles WHERE Status = 'Operational' LIMIT 1";
            $vehicleStmt = $connection->prepare($vehicleQuery);
            ...
            if (!$vehicleRow) {
                die("No available vehicles.");
            }

            

    // Existing reservation insertion query with added VehicleID and DriverID
    $sql_table = "INSERT INTO `reservation` (purpose, vehicle_type, cargo, destination_to, destination_from, schedule, arrival, departure, status, VehicleID, DriverID) 
                  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    
    if ($stmt = $connection->prepare($sql_table)) {
        $stmt->bind_param("ssssssssssi", $purpose, $vehicle_type, $cargo, $destination_to, $destination_from, $schedule, $arrival, $departure, $status, $vehicleID, $driverID);
        
        if ($stmt->execute()) {
            $updateVehicle = "UPDATE vehicles SET Status = 'Deployed' WHERE VehicleID = ?";
            $updateDriver = "UPDATE drivers SET Status = 'On duty' WHERE DriverID = ?";

            $stmtVehicle = $connection->prepare($updateVehicle);
            $stmtVehicle->bind_param("i", $vehicleID);
            $stmtVehicle->execute();
            $stmtVehicle->close();

            $stmtDriver = $connection->prepare($updateDriver);
...
// Query to fetch all reservation (reusing $result)
                ...
                <!-- All Content Put Here -->
                    ...
                    <div class="main-container">
                        ...
                        <div class="main-container-1">
                            ...
                            <span class="a">
                               <a href="sub-modules/completed_reservation.php">Complete reservation
                            ...
                            <span class="aaa">
                               <a href="sub-modules/failed_reservation.php">Failed reservation
                            ...
                            <span class="b">
                                <a href="sub-modules/dispatch.php">In transit
                            ...
                            <span class="c">
                                <a href="sub-modules/reservation_pending.php">Pending reservation
                            ...
                            <span class="d">
                                <a href="sub-modules/cancelled_reservation.php">Cancelled reservation
...
<div class="modal fade" id="vehicleReservationModal" tabindex="-1" aria-labelledby="modalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="vehicleReservationForm" method="POST" action="reservation.php">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select" id="purpose" name="purpose" onchange="toggleVehicleDropdown()">
                            ...
                            <option value="Guest">Guest Transport</option>
                            <option value="Staff">Staff Transport</option>
                            <option value="Logistic">Logistic Transport</option>
                        </select>
                    </div>

                    <!-- Vehicle Type Dropdown (Guest & Staff) -->
                    <div class="mb-3 vehicle-dropdown" id="guestStaffDropdown" style="display: none;">
                        <label for="vehicle_type" class="form-label"><i class="fas fa-car-side"></i> Vehicle Type</label>
                        <select class="form-select" id="vehicle_type" name="vehicle_type" onchange="calculateBill()">
                    ...
                    <!-- Vehicle Type Dropdown (Logistics) -->
                    <div class="mb-3 vehicle-dropdown" id="logisticDropdown" style="display: none;">
                        <label for="vehicle_type" class="form-label"><i class="fas fa-truck"></i> Vehicle Type</label>
                        <select class="form-select" id="vehicle_type" name="vehicle_type" onchange="toggleCargoDropdown(); calculateBill();">
                            <option selected disabled>Select Vehicle Type</option>
                    ...
                    <!-- Type of Cargo Dropdown (Only for Logistic Transport) -->
                    <div class="mb-3" id="cargoDropdown" style="display: none;">
                        ...
                        <select class="form-select" id="cargo" name="cargo">
                            ...
                            <option value="Liquid cargo">Fragile Items</option>
                        </select>
                    </div>
